...
fn plural_rules(c: &mut Criterion) {
  ...
  let langs: Vec<LanguageIdentifier> = langs.iter().map(|l| l.parse().expect("Parsing failed")).collect();

  c.bench_with_input(BenchmarkId::new("construct", langs.len()), &langs, |b, langs| {
    b.iter(|| {
      for lang in langs {
  ...
  let samples = &[
    1, 2, 3, 4, 5, 25, 134, 910293019, 12, 1412, -12, 15, 2931, 31231, 3123, 13231, 91, 0, 231, -2, -45, 33, 728, 2,
    291, 24, 479, 291, 778, 919, 93,
  ];

  let langid_pl = langid!("pl");
  let ipr = PluralRules::create(langid_pl.clone(), PluralRuleType::CARDINAL).unwrap();

  c.bench_with_input(BenchmarkId::new("select", samples.len()), samples, |b, samples| {
    b.iter(|| {
