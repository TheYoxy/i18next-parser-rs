...
fn range(i: &str) -> IResult<&str, Range> {
  map(separated_pair(value, tag(".."), value), |(lower_val, upper_val)| {
...
fn range_list(i: &str) -> IResult<&str, RangeList> {
  map(
    separated_list0(tuple((space0, tag(","), space0)), range_list_item),
    RangeList,
  )(i)
}
...
fn expression(i: &str) -> IResult<&str, Expression> {
  map(tuple((operand, mod_expression)), |(operand, modulus)| Expression {
    operand,
    modulus,
  })(i)
}
...
fn and_condition(i: &str) -> IResult<&str, AndCondition> {
  map(
    separated_list1(tuple((space1, tag("and"), space1)), relation),
    AndCondition,
  )(i)
}

fn decimal_value(i: &str) -> IResult<&str, DecimalValue> {
  map(tuple((value, opt(preceded(tag("."), value)))), |(integer, decimal)| {
    DecimalValue { integer, decimal }
  })(i)
}
...
pub fn parse_rule(i: &str) -> IResult<&str, Rule> {
  map(tuple((parse_condition, samples)), |(condition, samples)| Rule {
    condition,
    samples,
  })(i)
}
...
pub fn parse_condition(i: &str) -> IResult<&str, Condition> {
  ...
  map(
    separated_list1(tuple((space1, tag("or"), space1)), and_condition),
    Condition,
  )(i)
}
