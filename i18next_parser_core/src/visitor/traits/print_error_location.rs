...
pub fn get_line_bounds(text: &str, start_char_index: usize, end_char_index: usize) -> Option<(usize, usize)> {
...
pub trait PrintErrorLocation: OxcProgram {
  ...
  fn print_error_location(&self, _span: &oxc_span::Span) {
    #[cfg(feature = "print_error_location")]
    {
      ...
      let start_pos = usize::try_from(span.start).expect("span size overload");
      let end_pos = usize::try_from(span.end).expect("span size overload");
